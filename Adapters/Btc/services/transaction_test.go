...
func TestNodeClient_SendTx(t *testing.T) {
	...
	raw, err := nodeClient.GetNodeClient().CreateRawTx(ctx, []string{address1, address2}, address1, outs)
	if err != nil {
		log.Error(err)
		t.FailNow()
	}
	...
	signedTx, err := nodeClient.GetNodeClient().SignTransaction(ctx, raw, privates)
	if err != nil {
		log.Error(err)
		t.FailNow()
	}

	txId, err := nodeClient.GetNodeClient().SendTransaction(ctx, signedTx)
	if err != nil {
		log.Error(err)
		t.FailNow()
	}
...
func TestNodeClient_TransactionByHash(t *testing.T) {
	ctx, log := beforeTest()
	tx, err := nodeClient.GetNodeClient().TransactionByHash(ctx, "4d996fd7524c46a3afe959d0823ab15f30312bbf02e86a815749bca163b2664c")
	if err != nil {
		log.Error(err)
		t.FailNow()
	}
	...
	txUnknown, err := nodeClient.GetNodeClient().TransactionByHash(ctx, "4d996fd7524c46a3afe959d0823ab15f30312bbf02e86a815749bca163b26641")
	if err != nil {
		log.Error(err)
		t.FailNow()
	}
